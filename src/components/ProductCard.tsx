...
const ProductCard: React.FC<ProductCardProps> = ({ product, isTopThree, isInWishlist: isInWishlistProp }) => {
  ...
  const [isInWishlist, setIsInWishlist] = useState<boolean>(!!isInWishlistProp);
  ...
  useEffect(() => {
    if (typeof isInWishlistProp === 'boolean') {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        if (token && isInWishlistProp === undefined) {
          await fetchWishlistIds(token);
        }
      } catch (err) {
